...
- **背景图形**：左缘半露**数据天体**（canvas 正交投影**地形点云**引擎：~30k 颗地形粒子组成的自转球，密度跟地形走、**坑环致密粒子带**（亮带格点子点加密）+ 坑底疏粒盘 + 环形山**定向坡影**（迎光亮弧 / 背光暗口 / 坑碗影核三区光影）、加法混合微光、蓝火花点缀；双侧栏全收时半盘滑入 + **月面铭牌** PALIS 09A 报头）+ **星尘漂移场**（全屏微粒缓漂闪烁 + 近邻**星座连线**）+ HUD 几何层（虚线轨道环、十字准线、live 代码读数 UTC/LON/SIGNAL、表面粒子尘埃）+ 居中**环形轨道图**（含方位度数读数）+ **美术构成层**（月面铭牌标题 / 等高线地形碎片 / 测量十字 / 右缘 hex 数据流 / 错位图版 / 单色色卡阶梯 / 栏栅格碎片）+ 星尘底纹（参考 PALIS 自检/总目录屏；粒子路线参考 TouchDesigner）。
