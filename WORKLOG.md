...
## 65. 边框语言（0.5.13，2026-09-10）

- 用户方向：「继续优化当前 UI 边框的效果」。盘查先发现根问题：边框没有语言——灰线有
  #2a2a2a / #333 / W-.14 / W-.22 四档，强调蓝两档，各写各的。
- 四个 token 收敛语义：--palis-line-frame（外框/登记线，与 W2 同档）、--palis-line-div
  （分区线 #242424）、--palis-line-inset（内嵌细线 .10）、--palis-line-accent（活动/焦点）。
  边框一律 1px + 直角、全程不用阴影；深度来自内嵌细线而非投影。
- 四角标升级为"登记标记"：外层实线 + ::after 内嵌细线（inset 3px）构成双线 L。
- 分区线统一：状态栏顶边/段间竖线、hero 头部底边 → line-div；天体球壳边 → line-inset。
- 输入卡双层边：box-shadow inset 1px —— 外框内收一条暗线，读作"窗口"。
- 验证：计算样式实测（外层 .44 / 内层 .10@3px / 状态栏 #242424 / 输入卡 inset .1），
  硬编码灰线残留 0。又一次印证"别看图猜"：14× 抗锯齿图上我以为内层更亮，实测相反。
